...
tile with a flat diagonal gradient and a bold monogram. HealthLog swaps the
initials to **HL** and the palette to an Apple-Health-inspired pink→red
(`#FF6383` → `#E11D48`).

## Source of truth

The SVGs are authoritative; the PNG/ICO files are generated from them.
The monogram is drawn as vector rectangles (not `<text>`), so rendering does not
depend on any installed font.

| File | Purpose |
| --- | --- |
| `icon.svg` | Primary source — rounded tile, "HL" monogram |
| `icon-source-maskable.svg` | Full-bleed background, monogram scaled into the inner-80% safe zone |
...
| `proposal-bars.svg` | Rising bars with a trend arrow — statistical findings & trends |
