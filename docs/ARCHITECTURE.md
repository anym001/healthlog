...
```sql
findings (id, computed_at, kind TEXT,            -- correlation|anomaly|trend|seasonality|recovery_alert|consistency|training_load|training_status|stress|body_battery|weekly_*|fitness_markers
...
- **weekly_\*** — descriptive week summaries for the weekly report (`narrate
  --weekly`); status findings like `training_status`, written **every** run. All
  windows are trailing 7 days anchored on the **last day with any data** (shared
...
- **fitness_markers** — latest reading of the slow markers (VO2 Max, cardio
  ...
  older (`latest_marker_delta`); they move too slowly for weekly windows.
